...
public static class DuckDbLibrary
{
    ...
    /// Called by `AddDuckPg` before anything touches DuckDB, and callable directly by anything
    /// that reaches for the bindings first. Registering the resolver twice is an error, so it
    /// happens once however many times it is asked for.
