...
- **A deferred table is published as the merge, never as an empty one.** `Config.Lazy` finds the
  ...
  leaves the table deferred, and `Flush` skips what was never collapsed. A collapse while the lake
  serves is one transaction, and what it recorded is said after that commits; it happens *between*
  client transactions and never during one -- a transaction reads the catalog as it stood when it
  began -- and it carries the write branch the run has earned rather than only the one a file names.
